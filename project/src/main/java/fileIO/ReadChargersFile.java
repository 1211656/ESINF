    ...
    public static List<Supercharger> getDataFromFile(String file){
        ...
        try(BufferedReader csvReader = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = csvReader.readLine()) != null){
                ...
                superchargerList.add(new Supercharger(row[5].trim(), row[3].trim(), row[2].trim(), row[1].trim(), row[4].trim(), new Stalls(Integer.parseInt(row[6].trim())), new PowerKw(Integer.parseInt(row[7].trim())), new Elevm(Integer.parseInt(row[9].trim())), Status.createStatus(row[10].trim()), row[8].trim()));
                /*                          5                 3                  2                  1
